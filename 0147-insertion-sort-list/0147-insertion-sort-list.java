...
class Solution {
    public ListNode insertionSortList(ListNode head) {
        // Base case: if the list is empty or has only one element, it's already sorted ð
        ...
        // Step 1: Iterate through the unsorted linked list ð
        while (curr != null) {
            next = curr.next; // Save the future path before altering links
            
            // Step 2: Scan the sorted subset from the beginning to find the insert point ð
            while (prev.next != null && prev.next.val < curr.val) {
                prev = prev.next;
            }
            
            // Step 3: Splice 'curr' into its correct position between 'prev' and 'prev.next' âï¸
            curr.next = prev.next;
            prev.next = curr;
